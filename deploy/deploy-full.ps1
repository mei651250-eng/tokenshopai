...
if (Test-Path $ProjectDir) {
    Write-Host "目录已存在，拉取最新..." -ForegroundColor Yellow
    cd $ProjectDir; git pull
} else {
    git clone $RepoUrl $ProjectDir
}
cd $ProjectDir
Write-Host "代码 OK" -ForegroundColor Green
...
Write-Host "Go OK: $(go version)" -ForegroundColor Green
...
try {
    ...
    Write-Host "数据库初始化跳过: $_" -ForegroundColor Yellow
}
